...
public class UsbExpansion extends PlaceholderExpansion {
    ...
    public UsbExpansion(uSkyBlock usb, Path pluginDataDir) {
        this.usb = usb;
    ...
    public String onRequest(OfflinePlayer player, String params_) {
        ...
        /*
         * %uskyblock_istop_1_leadername%  -> name of the leader of island on rank 1
         * %uskyblock_istop_3_level%  -> level of island on rank 3
         * */

        if(params[0].equalsIgnoreCase("istop")) {
            // check if top10 even exists
            if(logic.getRanks(0, 1).isEmpty()) {
                return "unknown";
            }
            int place;
            try {
        ...
        IslandInfo isInfo = logic.getIslandInfo(new PlayerInfo(player.getName(), player.getUniqueId(), usb, pluginDataDir.resolve("players")));
